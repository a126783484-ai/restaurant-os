  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center justify-between gap-3">
        ...
        <div className={cn("flex h-10 w-10 items-center justify-center rounded-2xl shadow-sm", accent ?? "bg-primary/10")}>
...
function ActivityBadge({ type }: { type: string }) {
  ...
  return (
    <span className={cn("rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-wider", styles[type] ?? "bg-muted text-muted-foreground")}>
      {ACTIVITY_TYPE_LABELS[type] ?? type}
    </span>
  );
}

function QuickAction({ href, label, description, icon: Icon }: { href: string; label: string; description: string; icon: React.ElementType }) {
  return (
    <Link href={href}>
...
function OperatingPulse({ pendingOrders, activeReservations, todayOrders }: { pendingOrders: number; activeReservations: number; todayOrders: number }) {
  const loadLevel = pendingOrders >= 8 ? "busy" : pendingOrders >= 3 ? "active" : "calm";
  const label = loadLevel === "busy" ? "高峰處理中" : loadLevel === "active" ? "正常營運中" : "節奏穩定";
  const Icon = loadLevel === "busy" ? AlertTriangle : loadLevel === "active" ? Timer : CheckCircle2;

  return (
    <div className="rounded-[2rem] border border-card-border bg-card p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-muted-foreground">Operating Pulse</p>
          <h2 className="mt-2 text-xl font-black tracking-tight text-foreground">{label}</h2>
        </div>
        <div className={cn(
          "flex h-11 w-11 items-center justify-center rounded-2xl",
          loadLevel === "busy" ? "bg-amber-500/12 text-amber-600" : loadLevel === "active" ? "bg-primary/10 text-primary" : "bg-emerald-500/10 text-emerald-600",
        )}>
          <Icon className="h-5 w-5" />
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2">
        ...
        <div className="rounded-2xl bg-muted/60 p-3 text-center">
          <p className="text-lg font-black text-foreground">{activeReservations}</p>
    ...
    <div className="mx-auto max-w-7xl space-y-6 overflow-x-hidden p-4 sm:p-6">
      <section className="relative overflow-hidden rounded-[2rem] border border-border bg-card p-5 shadow-sm sm:p-6">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-2xl font-black tracking-tight text-foreground sm:text-4xl">今天的營運，一眼看清楚。</h1>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground sm:text-base">
              {today} · 從訂單、桌位、廚房、客流到營收，集中在同一個指揮中心判斷下一步。
            </p>
          </div>

          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:w-[520px]">
            <QuickAction href="/orders" label="處理訂單" description="開單、加菜、付款" icon={ShoppingBag} />
            <QuickAction href="/kitchen" label="看廚房" description="確認出餐節奏" icon={ChefHat} />
            <QuickAction href="/floor-plan" label="看桌位" description="安排入座翻桌" icon={LayoutGrid} />
          </div>
        </div>
      </section>
      ...
      {hasError && (
        <div className="rounded-3xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800 shadow-sm dark:bg-amber-950/30 dark:text-amber-200">
          部分營運資料暫時無法讀取；頁面仍會顯示已取得的資訊。請優先確認 API 與登入權限狀態。
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_340px]">
          ...
          {summaryLoading ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="h-36 animate-pulse rounded-3xl border border-card-border bg-card" />
              ))}
            </div>
          ) : summary ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <KPICard title="今日營業額" value={formatCurrency(summary.todaySales)} sub={`本週 ${formatCurrency(summary.weekSales)}`} icon={DollarSign} accent="bg-primary" tone="primary" />
              <KPICard title="今日訂單" value={String(summary.todayOrders)} sub={`${summary.pendingOrders} 筆待處理`} icon={ShoppingBag} tone={summary.pendingOrders > 0 ? "warning" : "default"} />
              <KPICard title="今日顧客" value={String(summary.todayCustomers)} sub="不重複訪客" icon={Users} />
              <KPICard title="回客率" value={`${summary.repeatCustomerRate.toFixed(1)}%`} sub="回頭客比例" icon={RefreshCw} tone="success" />
              <KPICard title="待處理訂單" value={String(summary.pendingOrders)} sub="前台與廚房優先處理" icon={Clock} tone={summary.pendingOrders > 0 ? "warning" : "success"} />
              <KPICard title="訂位數" value={String(summary.activeReservations)} sub="今日有效訂位" icon={CalendarDays} />
              <KPICard title="本週營收" value={formatCurrency(summary.weekSales)} sub="週期累積表現" icon={TrendingUp} />
              <KPICard title="系統狀態" value="運作中" sub="production ready" icon={Activity} tone="success" />
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <KPICard title="今日營業額" value="$0" sub="資料暫不可用" icon={DollarSign} accent="bg-primary" tone="primary" />
              <KPICard title="今日訂單" value="0" sub="請稍後重新整理" icon={ShoppingBag} />
              <KPICard title="進行中訂單" value="0" icon={Clock} />
              <KPICard title="系統狀態" value="需注意" sub="API 部分失敗" icon={Activity} tone="warning" />
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
            <div className="rounded-[2rem] border border-card-border bg-card p-5 shadow-sm lg:col-span-3">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-base font-black text-foreground">今日客流量</h2>
                  <p className="text-xs text-muted-foreground">依時段觀察尖峰與低谷</p>
                </div>
                <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-black text-primary">Live</span>
              </div>
              {flowLoading ? (
                <div className="h-60 animate-pulse rounded-3xl bg-muted" />
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <AreaChart data={flowError ? [] : (flow ?? [])} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                    <defs>
                    ...
                    <Tooltip contentStyle={{ borderRadius: "16px", fontSize: "12px", border: "1px solid hsl(var(--border))" }} formatter={(v) => [v, "顧客數"]} />
                    <Area type="monotone" dataKey="customers" stroke="hsl(var(--primary))" fill="url(#flowGrad)" strokeWidth={3} dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div className="rounded-[2rem] border border-card-border bg-card p-5 shadow-sm lg:col-span-2">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-base font-black text-foreground">熱銷商品</h2>
                  <p className="text-xs text-muted-foreground">今日優先備料參考</p>
                </div>
                <Link href="/products">
                  <span className="rounded-full bg-muted px-3 py-1 text-xs font-black text-foreground transition-colors hover:bg-primary hover:text-primary-foreground">菜單</span>
                </Link>
              </div>
              {productsLoading ? (
                <div className="space-y-3">
                  {Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-11 animate-pulse rounded-2xl bg-muted" />)}
                </div>
              ) : (
                <div className="space-y-3">
                  {(productsError ? [] : (topProducts ?? [])).slice(0, 6).map((p, i) => (
                    <div key={p.productId} className="flex items-center gap-3 rounded-2xl bg-muted/45 p-3">
                      <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-xs font-black", i === 0 ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground")}>
                        {i + 1}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-bold text-foreground">{p.productName}</p>
                        <p className="text-xs text-muted-foreground">已售 {p.totalSold} 份</p>
                      </div>
                      <span className="shrink-0 text-sm font-black text-foreground">{formatCurrency(p.totalRevenue)}</span>
                    </div>
                  ))}
                  {(productsError || !topProducts || topProducts.length === 0) && (
                    <p className="py-10 text-center text-sm text-muted-foreground">尚無銷售資料</p>
                  )}
                </div>
              )}
            </div>
        ...
        <aside className="space-y-6">
          <OperatingPulse pendingOrders={pendingOrders} activeReservations={activeReservations} todayOrders={todayOrders} />

          <div className="rounded-[2rem] border border-card-border bg-card p-5 shadow-sm">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-black text-foreground">最近動態</h2>
            ...
            {activityLoading ? (
              ...
              <div className="space-y-3">
                {(activity ?? []).slice(0, 7).map((item) => (
                  <div key={item.id} data-testid={`activity-${item.id}`} className="rounded-2xl border border-border/70 bg-muted/25 p-3">
                    ...
                    <p className="mt-2 text-sm font-medium leading-relaxed text-foreground">{item.description}</p>
                    {item.amount != null && (
                      <p className="mt-1 text-sm font-black text-primary">{formatCurrency(item.amount)}</p>
                    )}
                  </div>
                ))}
                {(activityError || !activity || activity.length === 0) && (
                  <p className="py-10 text-center text-sm text-muted-foreground">尚無最近動態</p>
                )}
              </div>
            )}
          </div>
