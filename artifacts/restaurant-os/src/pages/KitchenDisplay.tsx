...
import { AlertTriangle, ArrowRight, CheckCircle2, ChefHat, Clock, Radio, RefreshCw, Utensils } from "lucide-react";
...
function ElapsedTime({ createdAt }: { createdAt: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-black", elapsed >= 20 ? "bg-red-500 text-white" : elapsed >= 12 ? "bg-amber-500/15 text-amber-700 dark:text-amber-300" : "bg-background/70 text-muted-foreground")}>
...
function KDSCard({ orderId, onAdvance }: { orderId: number; onAdvance: () => void }) {
  ...
  return (
    <div className={cn("rounded-[1.75rem] border-2 p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg", cfg.card)}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-2xl font-black tracking-tight text-foreground">#{order.id}</span>
            <span className={cn("rounded-full px-2.5 py-1 text-xs font-black", cfg.badge)}>{STATUS_LABELS[order.status]}</span>
        ...
        <span className="text-lg font-black text-foreground shrink-0">${order.totalAmount.toFixed(0)}</span>
      </div>
      ...
      <div className="mt-4 divide-y divide-border/60 overflow-hidden rounded-2xl border border-white/60 bg-white/70 dark:border-white/10 dark:bg-black/25">
        {order.items?.length ? order.items.map((item) => (
          <div key={item.id} className="flex items-center justify-between gap-3 px-3 py-3">
            <span className="truncate text-base font-black text-foreground">{item.productName}</span>
            <span className="flex min-h-10 min-w-10 shrink-0 items-center justify-center rounded-xl bg-foreground px-2 text-lg font-black text-background">×{item.quantity}</span>
          </div>
        )) : (
          <div className="px-3 py-6 text-center text-sm font-medium text-muted-foreground">此訂單尚無品項資料</div>
        )}
      </div>
      ...
      {next && (
        <button
          ...
          className={cn("mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black transition-colors disabled:opacity-60", cfg.btn)}
        >
          {updateOrder.isPending ? "更新中…" : STATUS_ACTION_LABELS[order.status]}
...
function Column({ title, subtitle, badge, orderIds, onAdvance }: { title: string; subtitle: string; badge: string; orderIds: number[]; onAdvance: () => void }) {
  return (
    <div className="flex flex-col gap-4 min-w-0">
      <div className="sticky top-0 z-10 rounded-3xl border border-border bg-background/90 px-4 py-3 shadow-sm backdrop-blur-xl">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className={cn("flex h-9 min-w-9 items-center justify-center rounded-2xl px-3 text-sm font-black text-white", badge)}>{orderIds.length}</span>
        </div>
      </div>
      <div className="flex flex-col gap-3">
        {orderIds.length === 0 ? (
          <div className="rounded-[1.75rem] border-2 border-dashed border-border bg-card/60 p-8 text-center shadow-sm">
            ...
            <p className="mt-1 text-xs text-muted-foreground">沒有需要處理的訂單</p>
          </div>
        ) : (
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-border bg-background/90 backdrop-blur-xl shrink-0">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-xl font-black tracking-tight text-foreground">廚房出餐指揮台</h1>
          ...
          <Button variant="outline" size="sm" onClick={refresh} className="gap-1.5 min-h-11 rounded-2xl self-start lg:self-auto">
        ...
        <div className="mx-auto max-w-7xl">
          {error && (
            <div className="mb-4 rounded-3xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm font-bold text-destructive">
              <AlertTriangle className="mr-2 inline h-4 w-4" /> KDS 暫時無法讀取訂單，請重新整理。
            </div>
          )}
          {loading ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-64 rounded-[1.75rem] bg-muted animate-pulse" />)}
            </div>
          ) : total === 0 ? (
            <div className="rounded-[2rem] border border-border bg-card p-10 text-center shadow-sm">
              ...
              <h2 className="mt-4 text-xl font-black text-foreground">廚房目前沒有待處理訂單</h2>
              <p className="mt-2 text-sm text-muted-foreground">新訂單建立後會自動出現在這裡，每 30 秒同步一次。</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Column title="待處理" subtitle="新進訂單，等待開始製作" badge="bg-amber-500" orderIds={pendingIds} onAdvance={refresh} />
              <Column title="製作中" subtitle="廚房正在處理" badge="bg-blue-500" orderIds={preparingIds} onAdvance={refresh} />
              <Column title="可出餐" subtitle="等待送餐或取餐" badge="bg-emerald-500" orderIds={readyIds} onAdvance={refresh} />
            </div>
          )}
        </div>
      </div>
