...
export default function OrderReceipt() {
  ...
  if (isLoading) {
    return (
      <div className="p-6 text-sm text-muted-foreground">正在載入收據…</div>
    );
  }
      ...
      <div className="mx-auto max-w-2xl space-y-4">
        <div className="flex items-center justify-between gap-3 print:hidden">
          ...
          <Button
            ...
            className="gap-1.5 rounded-2xl font-black"
          >
        ...
        <main className="rounded-[2rem] border border-border bg-card p-6 shadow-sm print:rounded-none print:border-0 print:bg-white print:p-0 print:shadow-none">
          <header className="border-b border-border pb-5 text-center">
            ...
            <h1 className="mt-3 text-2xl font-black tracking-tight text-foreground print:mt-0">
              Restaurant OS
            </h1>
          ...
          <section className="space-y-2 border-b border-border py-5">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-foreground">
                {PAY_METHOD_LABELS[order.paymentMethod ?? "unpaid"] ??
                  order.paymentMethod ??
                  "未付款"}
              </span>
            </div>
          ...
          {(order.notes || order.paymentNote) && (
            <section className="border-b border-border py-5 text-sm">
              {order.notes && (
                <p>
                  <span className="font-black">訂單備註：</span>
                  {order.notes}
                </p>
              )}
              {order.paymentNote && (
                <p className="mt-2">
                  <span className="font-black">付款備註：</span>
                  {order.paymentNote}
                </p>
              )}
            </section>
          ...
          <footer className="pt-5 text-center text-xs text-muted-foreground">
            <p>感謝您的光臨</p>
            <p className="mt-1">
              Printed from Restaurant OS · {new Date().toLocaleString("zh-TW")}
            </p>
          </footer>
        </main>
      </div>
